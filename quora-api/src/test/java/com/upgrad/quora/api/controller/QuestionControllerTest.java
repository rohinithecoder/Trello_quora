...
public class QuestionControllerTest {
    ...
    public void createQuestionWithNonExistingAccessToken() throws Exception {
        mvc.perform(MockMvcRequestBuilders.post("/question/create").content( "{\"content\": \"my_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "non_existing_access_token"))
    ...
    public void createQuestionWithSignedOutUser() throws Exception {
        mvc.perform(MockMvcRequestBuilders.post("/question/create").content( "{\"content\": \"my_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "database_accesstoken3"))
                .andExpect(status().isForbidden())
    ...
    public void getAllQuestions() throws Exception {
        mvc.perform(MockMvcRequestBuilders.get("/question/all").header("authorization", "eyJraWQiOiJmNDI5OTJiOC0xZTk5LTQ3MGQtODQ2YS03N2Y2YzQzMDhjNDEiLCJ0eXAiOiJKV1QiLCJhbGciOiJIUzUxMiJ9.eyJhdWQiOiJlMTg4Yjk0OS0yOTJlLTRjMjUtYmQyOC1kNzlmYzdmNzg5MDMiLCJpc3MiOiJodHRwczovL3F1b3JhLmlvIiwiZXhwIjoxNjEyMDk1LCJpYXQiOjE2MTE0OTB9.WYg9PIUHCGJV_Ba4uPD6srkxeOOdRw4XfRumfePzoK1Oh71ZPDyRAzZ8zmO6xGQvU8rOHOjnerp86IDTZxemrw"))
    ...
    public void editQuestionWithNonExistingAccessToken() throws Exception {
        mvc.perform(MockMvcRequestBuilders.put("/question/edit/database_question_uuid").content( "{\"content\":\"edited_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "non_existing_access_token"))
                .andExpect(status().isForbidden())
                .andExpect(MockMvcResultMatchers.jsonPath("code").value("ATHR-001"));
    }
    ...
    public void editQuestionWithWithSignedOutUser() throws Exception {
        mvc.perform(MockMvcRequestBuilders.put("/question/edit/database_question_uuid").content( "{\"content\":\"edited_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "database_accesstoken3"))
                .andExpect(status().isForbidden())
                .andExpect(MockMvcResultMatchers.jsonPath("code").value("ATHR-002"));
    }
    ...
    public void editQuestionWithoutOwnership() throws Exception {
        mvc.perform(MockMvcRequestBuilders.put("/question/edit/database_question_uuid").content( "{\"content\":\"edited_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "database_accesstoken"))
                .andExpect(status().isForbidden())
    ...
    public void editNonExistingQuestion() throws Exception {
        mvc.perform(MockMvcRequestBuilders.put("/question/edit/non_exisitng_question_uuid").content( "{\"content\":\"edited_question\"}" ).contentType(MediaType.APPLICATION_JSON_UTF8_VALUE).header("authorization", "database_accesstoken1"))
